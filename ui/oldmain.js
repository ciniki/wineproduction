//
function ciniki_wineproduction_oldmain() {
    //
    // Panels
    //
    this.oldmain = null;
    ...
    this.initStart = function() {
        var rsp = M.api.getJSONCb('ciniki.wineproduction.settingsGet', 
            {'tnid':M.curTenantID}, function(rsp) {
                ...
                var o = M.ciniki_wineproduction_oldmain;
                ...
                M.ciniki_wineproduction_oldmain.initFinish();
            });
    };
    ...
    this.initFinish = function() {
        ...
        //
        this.oldmain = new M.panel('Wine Production',
            'ciniki_wineproduction_oldmain', 'oldmain',
            'mc', 'medium', 'sectioned', 'ciniki.wineproduction.oldmain');
        this.oldmain.sections = {
            ...
            'today':{'label':'Todays Production', 'type':'simplelist', 'list':{
                'start':{'label':'Starting', 'precount':'no', 'count':0, 'fn':'M.ciniki_wineproduction_oldmain.showOrders(null, \'start\');'},
                'todayssgreadings':{'label':'SG Readings', 'precount':'no', 'count':0, 'fn':'M.ciniki_wineproduction_oldmain.showOrders(null, \'todayssgreadings\');'},
                'todaysracking':{'label':'Racking', 'precount':'no', 'count':0, 'fn':'M.ciniki_wineproduction_oldmain.showOrders(null, \'todaysracking\');'},
                'todaysfiltering':{'label':'Filtering', 'precount':'no', 'count':0, 'fn':'M.ciniki_wineproduction_oldmain.showOrders(null, \'todaysfiltering\');'},
                }},
            'orders':{'label':'Orders', 'type':'simplelist', 'list':{
                'ordered':{'label':'Ordered', 'count':0, 'fn':'M.ciniki_wineproduction_oldmain.showOrders(null, \'ordered\');'},
                'started':{'label':'Started', 'count':0, 'fn':'M.ciniki_wineproduction_oldmain.showOrders(null, \'started\');'},
                'sgready':{'label':'SG Ready', 'count':0, 'fn':'M.ciniki_wineproduction_oldmain.showOrders(null, \'sgready\');'},
                'racked':{'label':'Racked', 'count':0, 'fn':'M.ciniki_wineproduction_oldmain.showOrders(null, \'racked\');'},
                'filtered':{'label':'Filtered', 'count':0, 'fn':'M.ciniki_wineproduction_oldmain.showOrders(null, \'filtered\');'},
                'shared':{'label':'Shared', 'count':0, 'fn':'M.ciniki_wineproduction_oldmain.showOrders(null, \'shared\');'},
                }},
            'reports':{'label':'Reports', 'type':'simplelist', 'list':{
                'schedule':{'label':'Production Schedule', 'fn':'M.ciniki_wineproduction_oldmain.showSchedule(\'M.ciniki_wineproduction_oldmain.showMain();\',null,\'today\');'},
                'workdone':{'label':'Work Completed', 'count':0, 'fn':'M.ciniki_wineproduction_oldmain.showWorkDone(\'today\', \'all\');'},
                'latewines':{'label':'Late Wines', 'count':0, 'fn':'M.ciniki_wineproduction_oldmain.showOrders(null, \'latewines\');'},
                'ctb':{'label':'Call to Book', 'count':0, 'fn':'M.ciniki_wineproduction_oldmain.showOrders(\'M.ciniki_wineproduction_oldmain.showMain();\', \'ctb\');'},
                'cellarnights':{'label':'Cellar Nights', 'fn':'M.startApp(\'ciniki.wineproduction.reports\',null,\'M.ciniki_wineproduction_oldmain.showMain();\',\'mc\',{\'report\':\'cellarnights\'});'},
                'shared':{'label':'Shared Batches', 'fn':'M.startApp(\'ciniki.wineproduction.reports\',null,\'M.ciniki_wineproduction_oldmain.showMain();\',\'mc\',{\'report\':\'shared\'});'},
                'customerhistory':{'label':'Yearly Report', 'fn':'M.startApp(\'ciniki.wineproduction.reports\',null,\'M.ciniki_wineproduction_oldmain.showMain();\',\'mc\',{\'report\':\'customers_yearly\'});'},
                'export':{'label':'Export Orders', 'fn':'M.ciniki_wineproduction_oldmain.downloadXLS();'},
                }},
            };
        this.oldmain.listValue = function(s, i, d) { 
        ...
        this.oldmain.listFn = function(s, i, d) { 
            if( d.fn != null ) { return d.fn; } 
            return '';
        };
        this.oldmain.fieldValue = function(s, i, d) { return ''; }
        // FIXME: Also resides in core/js/menu.js
        this.oldmain.liveSearchCb = function(s, i, value) {
            if( s == 'search' && value != '' ) {
                M.api.getJSONBgCb('ciniki.wineproduction.searchQuick', {'tnid':M.curTenantID, 'start_needle':value, 'limit':'10'}, 
                    function(rsp) { 
                        M.ciniki_wineproduction_oldmain.oldmain.liveSearchShow('search', null, M.gE(M.ciniki_wineproduction_oldmain.oldmain.panelUID + '_' + s), rsp.orders); 
                    });
                return true;
            }
        };
        this.oldmain.liveSearchResultClass = function(s, f, i, j, d) {
            if( j > 2 ) { return 'multiline aligncenter'; }
            return 'multiline';
        };
        this.oldmain.liveSearchResultValue = function(s, f, i, j, d) {
            if( s == 'search' ) {
                if( j == 0 ) {
                    return "<span class='maintext'>" + d.order.invoice_number + "</span>" + "<span class='subtext'>" + M.ciniki_wineproduction_oldmain.statusOptions[d.order.status] + "</span>";
                }
        ...
        this.oldmain.liveSearchResultRowFn = function(s, f, i, j, d) { 
            return 'M.ciniki_wineproduction_oldmain.showOrder(\'' + d.order.id + '\', \'M.ciniki_wineproduction_oldmain.oldmain.show();\');'; 
        };
        this.oldmain.liveSearchSubmitFn = function(s, search_str) {
            M.ciniki_wineproduction_oldmain.search.cb = 'M.ciniki_wineproduction_oldmain.showMain();';
            M.ciniki_wineproduction_oldmain.searchOrders(search_str);
        };

        this.oldmain.addButton('add', 'Add', 'M.ciniki_wineproduction_oldmain.showAdd(\'M.ciniki_wineproduction_oldmain.showMain();\');');
        this.oldmain.addClose('Back');

        //
        // The form panel to add a new production order
        //
        this.add = new M.panel('Add Wine Order',
            'ciniki_wineproduction_oldmain', 'add',
            'mc', 'medium', 'sectioned', 'ciniki.wineproduction.oldmain.edit');
        this.add.data = {};
        this.add.sections = {
            'info':{'label':'', 'fields':{
            ...
            '_buttons':{'label':'', 'buttons':{
                'save':{'label':'Save order', 'fn':'M.ciniki_wineproduction_oldmain.addOrder();'},
                }},
            };
        this.add.fieldValue = function(s, i, d) { 
            if( i == 'bottling_duration' ) { return '60'; }
            if( i == 'order_date' ) {
                return M.ciniki_wineproduction_oldmain.dateToday;
            }
            return ''; 
        };
        ...
        this.add.liveSearchCb = function(s, i, value) {
            if( i.match(/^product_id/) ) {
                ...
                if( value != '' || cv > 0 ) {
                    var rsp = M.api.getJSONBgCb('ciniki.wineproduction.searchProductNames', 
                        ...
                        function(rsp) { 
                            M.ciniki_wineproduction_oldmain.add.liveSearchShow(s, i, M.gE(M.ciniki_wineproduction_oldmain.add.panelUID + '_' + i), rsp.names); 
                        });
                }
            }
            if( i == 'customer_id' && value != '' ) {
                var rsp = M.api.getJSONBgCb('ciniki.customers.searchQuick', {'tnid':M.curTenantID, 'start_needle':value, 'limit':25},
                    function(rsp) { 
                        M.ciniki_wineproduction_oldmain.add.liveSearchShow(s, i, M.gE(M.ciniki_wineproduction_oldmain.add.panelUID + '_' + i), rsp.customers); 
                    });
            }
        };
        ...
        this.add.liveSearchResultRowFn = function(s, f, i, j, d) { 
            if( f.match(/^product_id/) ) {
                ...
                return 'M.ciniki_wineproduction_oldmain.add.updateProduct(\'' + s + '\',\'' + x + '\',\'' + d.name.id + '\',\'' + escape(d.name.wine_name) + '\',\'' + d.name.wine_type + '\',\'' + d.name.kit_length + '\',\'' + d.name.order_flags + '\');';
            } else if( f == 'customer_id' ) {
                return 'M.ciniki_wineproduction_oldmain.add.updateCustomer(\'' + s + '\',\'' + escape(d.display_name) + '\',\'' + d.id + '\');';
            }
        };
...
//          return '#aaddff';
//      };
        // this.add.addLeftButton('cancel', 'Cancel', 'M.ciniki_wineproduction_oldmain.showMain();');
        this.add.addButton('save', 'Save', 'M.ciniki_wineproduction_oldmain.addOrder();');
        ...
        this.list = new M.panel('Wine List',
            'ciniki_wineproduction_oldmain', 'list',
            'mc', 'xlarge', 'sectioned', 'ciniki.wineproduction.oldmain.list');
        this.list.gridSorting = {};
        this.list.sections = {
            'pending':{'label':'', 'num_cols':1, 'headerValues':[], 'sortable':'yes', 'searchable':'yes', 'savesort':'M.ciniki_wineproduction_oldmain.list.saveSortOrder', 'type':'simplegrid'},
            'completed':{'label':'', 'num_cols':1, 'headerValues':[], 'sortable':'yes', 'savesort':'M.ciniki_wineproduction_oldmain.list.saveSortOrder', 'visible':'no', 'type':'simplegrid'},
            };
        //
        ...
        this.list.saveSortOrder = function(tid, c, t, o) {
            ...
            if( M.ciniki_wineproduction_oldmain.list.gridSorting[tid] == null ) {
                M.ciniki_wineproduction_oldmain.list.gridSorting[tid] = {};
            }
            M.ciniki_wineproduction_oldmain.list.gridSorting[tid][M.ciniki_wineproduction_oldmain.list.ordertype] = {'col':c, 'type':t, 'order':o};
        };
        ...
        this.list.rowStyle = function(section, i, d) {
            if( this.title == 'Call to Book' ) {
                for(j in M.ciniki_wineproduction_oldmain.bottlingStatus) {
                    if( (d.order.bottling_status&Math.pow(2, j-1)) == Math.pow(2,j-1) 
                        && M.ciniki_wineproduction_oldmain.bottlingStatus[j].bgcolour != '' 
                        && M.ciniki_wineproduction_oldmain.bottlingStatus[j].bgcolour != '#ffffff' ) {
                        return 'background: ' + M.ciniki_wineproduction_oldmain.bottlingStatus[j].bgcolour;
                    }
                }
//              for(j in M.ciniki_wineproduction_oldmain.bottlingFlags) {
//                  if( (d['order']['bottling_flags']&Math.pow(2, j-1)) == Math.pow(2,j-1) 
//                      && M.ciniki_wineproduction_oldmain.bottlingFlags[j].bgcolour != '' 
//                      && M.ciniki_wineproduction_oldmain.bottlingFlags[j].bgcolour != '#ffffff' ) {
//                      return 'background: ' + M.ciniki_wineproduction_oldmain.bottlingFlags[j].bgcolour;
//                  }
//              }
            }
            if( d.order.order_flags > 0 ) {
                for(j in M.ciniki_wineproduction_oldmain.orderFlags) {
                    if( (d.order.order_flags&Math.pow(2, j-1)) == Math.pow(2,j-1) 
                        && M.ciniki_wineproduction_oldmain.orderFlags[j].bgcolour != '' 
                        && M.ciniki_wineproduction_oldmain.orderFlags[j].bgcolour != '#ffffff' ) {
                        return 'background: ' + M.ciniki_wineproduction_oldmain.orderFlags[j].bgcolour;
                    }
                }
            }
            ...
            if( this.dataMaps[section][col] == 'bottling_flags_colour' ) {
                for(j in M.ciniki_wineproduction_oldmain.bottlingFlags) {
                    if( (d.order.bottling_flags&Math.pow(2, j-1)) == Math.pow(2,j-1) 
                        ...
                        && M.ciniki_wineproduction_oldmain.bottlingFlags[j].bgcolour != '#ffffff' ) {
                        // return 'background: ' + M.ciniki_wineproduction_oldmain.bottlingFlags[j].bgcolour;
                        return "<span class='colourswatch' style='background-color:" + M.ciniki_wineproduction_oldmain.bottlingFlags[j].bgcolour + ";'>&nbsp;</span>";
                    }
                }
                ...
                if( d.order.order_flags > 0 ) {
                    ...
                    for(j in M.ciniki_wineproduction_oldmain.orderFlags) {
                        if( (d.order.order_flags&Math.pow(2, j-1)) == Math.pow(2,j-1) ) {
                            if( flags != '' ) { flags += ', '; }
                            flags += M.ciniki_wineproduction_oldmain.orderFlags[j].name;
                        }
                    }
                ...
                if( d.order.bottling_flags > 0 ) {
                    ...
                    for(j in M.ciniki_wineproduction_oldmain.bottlingFlags) {
                        if( (d.order.bottling_flags&Math.pow(2, j-1)) == Math.pow(2,j-1) ) {
                            if( flags != '' ) { flags += ', '; }
                            flags += M.ciniki_wineproduction_oldmain.bottlingFlags[j].name;
                        }
                    }
                ...
                if( M.ciniki_wineproduction_oldmain.statusOptions[d.order.status] != null ) {
                    return M.ciniki_wineproduction_oldmain.statusOptions[d.order.status];
                } else {
                    return 'Unknown';
                }
                ...
                if( this.buttonText != 'Racked' || (this.buttonText == 'Racked' && d.order.status == 25) || (this.buttonText == 'Racked' && sg != '' 
                    ...
                    return "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.actionOrder('" + d.order.id + "', '" + this.buttonText + "','" + d.order.kit_length + "'); return false;\">" + this.buttonText + "</button>";
                }
            } else if( this.dataMaps[section][col] == 'sgbuttons' ) {
                return "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.quickSGbutton('" + d.order.id + "','992'); return false;\">92</button>" + 
                    "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.quickSGbutton('" + d.order.id + "','993'); return false;\">93</button>" +
                    "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.quickSGbutton('" + d.order.id + "','994'); return false;\">94</button>" +
                    "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.quickSGbutton('" + d.order.id + "','995'); return false;\">95</button>" +
                    "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.quickSGbutton('" + d.order.id + "','996'); return false;\">96</button>" +
                    "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.quickSGbutton('" + d.order.id + "','997'); return false;\">97</button>" +
                    "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.quickSGbutton('" + d.order.id + "','998'); return false;\">98</button>" +
                    "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.quickSGbutton('" + d.order.id + "','999'); return false;\">99</button>" +
                    "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.quickSGbutton('" + d.order.id + "','1000'); return false;\">00</button>" +
                    "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.quickSGbutton('" + d.order.id + "','1001'); return false;\">01</button>" +
                    "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.quickSGbutton('" + d.order.id + "','1002'); return false;\">02</button>";
        ...
        this.list.cellUpdateFn = function(section, row, col, d) {
            ...
            if( this.dataMaps[section][col] == 'sg_reading' ) {
                return M.ciniki_wineproduction_oldmain.updateSG;
            }
        }

        this.list.rowFn = function(s, i, d) { 
            if( this.title == 'Late Wines' ) {
                return 'M.ciniki_wineproduction_oldmain.showAppointment(\'M.ciniki_wineproduction_oldmain.showOrders(null, \\\'latewines\\\');\',\'' + d.order.appointment_id + '\');';
            }
            if( this.title == 'Call to Book' ) {
                return 'M.ciniki_wineproduction_oldmain.showAppointment(\'M.ciniki_wineproduction_oldmain.showOrders(null, \\\'ctb\\\');\',\'' + d.order.appointment_id + '\');';
            }
            return 'M.ciniki_wineproduction_oldmain.showOrder(\'' + d.order.id + '\', \'M.ciniki_wineproduction_oldmain.showOrders(null, M.ciniki_wineproduction_oldmain.list.ordertype);\');'; }
        ...
        //this.list.addLeftButton('back', 'Back', 'M.ciniki_wineproduction_oldmain.showMain();');
        ...
        this.workdone = new M.panel('Work Completed',
            'ciniki_wineproduction_oldmain', 'workdone',
            'mc', 'xlarge', 'sectioned', 'ciniki.wineproduction.oldmain.workdone');
        ...
        this.workdone.sections = {
            'datepicker':{'label':'', 'type':'datepicker', 'fn':'M.ciniki_wineproduction_oldmain.showWorkDoneCalFn'},
        ...
        this.workdone.rowFn = function(section, i, d) { return 'M.ciniki_wineproduction_oldmain.showOrder(\'' + d['order']['id'] + '\', \'M.ciniki_wineproduction_oldmain.showWorkDone(null, null);\');'; }
        ...
        this.workdone.addLeftButton('back', 'Back', 'M.ciniki_wineproduction_oldmain.showMain();');

        //
        // Display a current order
        //
        this.order = new M.panel('Wine Order',
            'ciniki_wineproduction_oldmain', 'order',
            'mc', 'medium mediumaside', 'sectioned', 'ciniki.wineproduction.oldmain.edit');
        ...
        this.order.sections = {
            'customer':{'label':'Customer', 'aside':'yes', 'type':'simplegrid', 'num_cols':2,
                ...
                'cellClasses':['label',''],
                'addTxt':'Edit',
                'addFn':'M.startApp(\'ciniki.customers.edit\',null,\'M.ciniki_wineproduction_oldmain.showOrder();\',\'mc\',{\'next\':\'M.ciniki_wineproduction_oldmain.updateOrderCustomer\',\'customer_id\':M.ciniki_wineproduction_oldmain.order.data.customer_id});',
                'changeTxt':'Change customer',
                'changeFn':'M.startApp(\'ciniki.customers.edit\',null,\'M.ciniki_wineproduction_oldmain.showOrder();\',\'mc\',{\'next\':\'M.ciniki_wineproduction_oldmain.updateOrderCustomer\',\'customer_id\':0});',
            },
...
//              'bottling_nocolour_flags':{'label':'Other', 'join':'yes', 'toggle':'no', 'type':'flags', 'flags':this.bottlingNocolourFlags},
            ...
            '_buttons':{'label':'', 'buttons':{
                'save':{'label':'Save order', 'fn':'M.ciniki_wineproduction_oldmain.saveOrder();'},
                }},
            };
        ...
        this.order.calBgColour = function(i, y, m, d) {
            ...
            //
            if( i == 'racking_date' ) {
                return M.ciniki_wineproduction_oldmain.settings['racking.autocolour.week' + (Math.floor(((c.getTime()/1000) - 1468800)/604800))%3 + M.dayOfWeek(c)];
            } else if( i == 'filtering_date' ) {
                
                return M.ciniki_wineproduction_oldmain.settings['filtering.autocolour.week' + (Math.floor(((c.getTime()/1000) - 1468800)/604800))%7 + M.dayOfWeek(c)];
            }
        };
        ...
        this.order.liveSearchCb = function(s, i, value) {
            if( i == 'product_id' ) {
                ...
                var rsp = M.api.getJSONBgCb('ciniki.wineproduction.searchProductNames', 
                    {'tnid':M.curTenantID, 'customer_id':cv, 'start_needle':value, 'limit':11},
                    function(rsp) { 
                        M.ciniki_wineproduction_oldmain.order.liveSearchShow(s, i, M.gE(M.ciniki_wineproduction_oldmain.order.panelUID + '_' + i), rsp['names']); 
                    });
            }
//          if( i == 'customer_id' && value != '' ) {
//              var rsp = M.api.getJSONBgCb('ciniki.customers.searchQuick', {'tnid':M.curTenantID, 'start_needle':value, 'limit':25},
//                  function(rsp) { 
//                      M.ciniki_wineproduction_oldmain.order.liveSearchShow(s, i, M.gE(M.ciniki_wineproduction_oldmain.order.panelUID + '_' + i), rsp['customers']); 
//                  });
//          }
        };
        ...
        this.order.liveSearchResultRowFn = function(s, f, i, j, d) { 
            if( f == 'product_id' ) {
                return 'M.ciniki_wineproduction_oldmain.order.updateProduct(\'' + s + '\',\'' + f + '\',\'' + d.name.id + '\',\'' + escape(d.name.wine_name) + '\',\'' + d.name.wine_type + '\',\'' + d.name.kit_length + '\',\'' + d.name.order_flags + '\');';
//          } else if( f == 'customer_id' ) {
//              return 'M.ciniki_wineproduction_oldmain.order.updateCustomer(\'' + s + '\',\'' + escape(d.customer.name) + '\',\'' + d.customer.id + '\');';
            }
        };
...
//      };
        ...
        this.order.addButton('save', 'Save', 'M.ciniki_wineproduction_oldmain.saveOrder();');
        this.order.addLeftButton('cancel', 'Cancel', 'M.ciniki_wineproduction_oldmain.order.close();');

        //
        // Then to display an bottling appointment
        //
        this.appointment = new M.panel('Appointment',
            'ciniki_wineproduction_oldmain', 'appointment',
            'mc', 'large mediumaside', 'sectioned', 'ciniki.wineproduction.oldmain.appointment');
        ...
        this.appointment.sections = {
            'customer':{'label':'Customer', 'type':'simplegrid', 'num_cols':2, 'aside':'yes',
                'cellClasses':['label',''],
                'addTxt':'Edit',
//                'addFn':'M.startApp(\'ciniki.customers.edit\',null,\'M.ciniki_wineproduction_oldmain.showAppointment();\',\'mc\',{\'next\':\'M.ciniki_wineproduction_oldmain.updateAppointmentCustomer\',\'customer_id\':M.ciniki_wineproduction_oldmain.appointment.customer_id});',
                'addFn':'M.startApp(\'ciniki.customers.main\',null,\'M.ciniki_wineproduction_oldmain.showAppointment();\',\'mc\',{\'next\':\'M.ciniki_wineproduction_oldmain.updateAppointmentCustomer\',\'customer_id\':M.ciniki_wineproduction_oldmain.appointment.customer_id});',
                'changeTxt':'Change customer',
                'changeFn':'M.startApp(\'ciniki.customers.edit\',null,\'M.ciniki_wineproduction_oldmain.showAppointment();\',\'mc\',{\'next\':\'M.ciniki_wineproduction_oldmain.updateAppointmentCustomer\',\'customer_id\':0});',
            },
...
//              'customer_name':{'label':'Customer', 'type':'noedit', 'size':'medium', 'history':'no'},
            ...
            '_bottled':{'label':'', 'buttons':{
                'bottled':{'label':'Bottled', 'fn':'M.ciniki_wineproduction_oldmain.saveAppointment(\'yes\');'},
            }},
            ...
            '_buttons':{'label':'', 'buttons':{
                'save':{'label':'Save', 'fn':'M.ciniki_wineproduction_oldmain.saveAppointment(\'no\');'},
            }},
        };
        ...
        this.appointment.rowFn = function(s, i, d) {
            if( s == 'wines' ) {
                return 'M.ciniki_wineproduction_oldmain.showOrder(\'' + d.order.order_id + '\', \'M.ciniki_wineproduction_oldmain.showAppointment(null, null);\');'; 
            }
            return '';
        };
        ...
        this.appointment.addButton('save', 'Save', 'M.ciniki_wineproduction_oldmain.saveAppointment(\'no\');');
        ...
        this.search = new M.panel('Search Results',
            'ciniki_wineproduction_oldmain', 'search',
            'mc', 'xlarge', 'sectioned', 'ciniki.wineproduction.oldmain.search');
        ...
        this.search.rowFn = function(s, i, d) { return 'M.ciniki_wineproduction_oldmain.showOrder(\'' + d.order.id + '\', \'M.ciniki_wineproduction_oldmain.search.show();\');'; }
        // this.search.addLeftButton('back', 'Back', 'M.ciniki_wineproduction_oldmain.showMain();');
        ...
        this.schedule = new M.panel('Schedule',
            'ciniki_wineproduction_oldmain', 'schedule',
            'mc', 'xlarge', 'sectioned', 'ciniki.wineproduction.oldmain.schedule');
                ...
                if( this.dataMaps[section][col] == 'bottling_flags_colour' ) {
                    ...
                    if( d.order.bottling_flags > 0 || d.order.bottling_status > 0 ) {
                        ...
                        for(j in M.ciniki_wineproduction_oldmain.bottlingStatus) {
                            if( (d['order']['bottling_status']&Math.pow(2, j-1)) == Math.pow(2,j-1) ) {
                                if( flags != '' ) { flags += ', '; }
                                flags += M.ciniki_wineproduction_oldmain.bottlingStatus[j].name;
                            }
                        }
                        return "<span class='maintext'>" + value + "</span>" + "<span class='subtext'>" + flags + "</span>";
                    }
                    ...
                    if( this.buttonText == 'Filter Today' && d.order.status == 40 ) {
                        ...
                        return "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.actionScheduleOrder('" + d.order.id + "', 'Bottled','" + d.order.kit_length + "'); return false;\">Bottled</button>";
                    } else if( this.buttonText != 'Racked' || (this.buttonText == 'Racked' && sg != '' 
                        && ((sg > .990 && sg < .999) || (sg > 990 && sg < 999))) ) {
                        return "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.actionScheduleOrder('" + d.order.id + "', '" + this.buttonText + "','" + d.order.kit_length + "'); return false;\">" + this.buttonText + "</button>";
                    } else if( this.buttonText == 'Racked' && sg == '' ) {
                        // Display SG Buttons
                        return "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.actionScheduleSGbutton('" + d.order.id + "','992'); return false;\">92</button>" + 
                            "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.actionScheduleSGbutton('" + d.order.id + "','993'); return false;\">93</button>" +
                            "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.actionScheduleSGbutton('" + d.order.id + "','994'); return false;\">94</button>" +
                            "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.actionScheduleSGbutton('" + d.order.id + "','995'); return false;\">95</button>" +
                            "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.actionScheduleSGbutton('" + d.order.id + "','996'); return false;\">96</button>" +
                            "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.actionScheduleSGbutton('" + d.order.id + "','997'); return false;\">97</button>" +
                            "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.actionScheduleSGbutton('" + d.order.id + "','998'); return false;\">98</button>" +
                            "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.actionScheduleSGbutton('" + d.order.id + "','999'); return false;\">99</button>" +
                            "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.actionScheduleSGbutton('" + d.order.id + "','1000'); return false;\">00</button>" +
                            "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.actionScheduleSGbutton('" + d.order.id + "','1001'); return false;\">01</button>" +
                            "<button onclick=\"event.stopPropagation(); M.ciniki_wineproduction_oldmain.actionScheduleSGbutton('" + d.order.id + "','1002'); return false;\">02</button>";
                    }
                } else {
                    var value = d.order[this.dataMaps[section][col]];
                    if( value == '0000-00-00' ) {
        ...
        this.schedule.cellFn = function(section, i, col, d) {
            ...
            if( section == 'stats' && (i == 'racking' || i == 'sgready' || i == 'filtering' || i == 'bottling' ) ) {
                if( (col > 1 && col < 16) && this.data['stats'][i][(col-2)]['stat']['count'] > 0 ) {
                    return 'M.ciniki_wineproduction_oldmain.showScheduleList(\'' + i + '\',\'' + this.data['stats'][i][(col-2)]['stat']['year'] + '-' + this.data['stats'][i][(col-2)]['stat']['month'] + '-' + this.data['stats'][i][(col-2)]['stat']['day'] + '\',\'' + this.data['stats'][i][(col-2)]['stat'][i+'_date'] + '\');';
                } else if( col == 1 && d['past'] > 0 ) { 
                    return 'M.ciniki_wineproduction_oldmain.showScheduleList(\'before_' + i + '\',\'' + this.data['stats'][i][(col-1)]['stat']['year'] + '-' + this.data['stats'][i][(col-1)]['stat']['month'] + '-' + this.data['stats'][i][(col-1)]['stat']['day'] + '\',\'Past\');';
                } else if( col == 16 && d['future'] > 0 ) {
                    return 'M.ciniki_wineproduction_oldmain.showScheduleList(\'after_' + i + '\',\'' + this.data['stats'][i][(col-3)]['stat']['year'] + '-' + this.data['stats'][i][(col-3)]['stat']['month'] + '-' + this.data['stats'][i][(col-3)]['stat']['day'] + '\',\'Future\');';
                }
            } 
            return '';
        };
        ...
        this.schedule.rowFn = function(section, i, d) { 
            if( section == 'orders' ) {
                return 'M.ciniki_wineproduction_oldmain.showOrder(\'' + d.order.id + '\', \'M.ciniki_wineproduction_oldmain.showSchedule();\');'; 
            }
            return null;
        };
...
//      this.schedule.addLeftButton('back', 'Back', 'M.ciniki_wineproduction_oldmain.showMain();');
    ...
    this.start = function(cb, appPrefix, aG) {
        ...
        // if it does exist.
        //
        var appContainer = M.createContainer(appPrefix, 'ciniki_wineproduction_oldmain', 'yes');
    ...
    this.showMain = function(cb) {
        var rsp = M.api.getJSONCb('ciniki.wineproduction.stats', 
            {'tnid':M.curTenantID}, function(rsp) { M.ciniki_wineproduction_oldmain.showMainFinish(cb, rsp); });
    };
    ...
    this.showMainFinish = function(cb, rsp) {
        ...
        var p = M.ciniki_wineproduction_oldmain.oldmain;
        p.stats = rsp.stats;
        //
    ...
    this.showWorkDone = function(workdate, worklist) {
        ...
        if( this.workdone.worklist == 'all' || this.workdone.worklist == 'started' ) {
            ...
            if( M.modFlagOn('ciniki.wineproduction', 0x0200) ) {
                ...
                this.workdone.sections['started'].num_cols = 5;
                this.workdone.sections['started'].headerValues = ['INV#', 'Wine', 'Ordered', 'BD', ''];
                this.workdone.sections['started'].sortTypes = ['number', 'text', 'date', 'date', 'none'];
                this.workdone.dataMaps['started'] = ['invoice_number', 'wine_name', 'order_date', 'bottling_date', 'notes'];
            }
        } else {
        ...
        if( this.workdone.worklist == 'all' || this.workdone.worklist == 'racked' ) {
            ...
            if( M.modFlagOn('ciniki.wineproduction', 0x0200) ) {
                ...
                this.workdone.sections['racked'].num_cols = 7;
                this.workdone.sections['racked'].headerValues = ['INV#', 'Wine', 'SG', 'Started', 'BD', 'RD', '', ''];
                this.workdone.sections['racked'].sortTypes = ['number', 'text', 'text', 'date', 'date', 'date', 'none'];
                this.workdone.dataMaps['racked'] = ['invoice_number', 'wine_name', 'sg_reading', 'start_date', 'bottling_date', 'racking_date', 'notes'];
            }
        } else {
        ...
        if( this.workdone.worklist == 'all' || this.workdone.worklist == 'filtered' ) {
            ...
            if( M.modFlagOn('ciniki.wineproduction', 0x0200) ) {
                ...
                this.workdone.sections['filtered'].num_cols = 6;
                this.workdone.sections['filtered'].headerValues = ['INV#', 'Wine', 'Racked', 'BD', 'FD', ''];
                this.workdone.sections['filtered'].sortTypes = ['number', 'text', 'date', 'date', 'date', 'none'];
                this.workdone.dataMaps['filtered'] = ['invoice_number', 'wine_name', 'rack_date', 'bottling_date', 'filtering_date', 'notes'];
            }
        } else {
...
//              rsp = M.api.getJSON('ciniki.wineproduction.list', {'tnid':M.curTenantID, 'started_date':this.workdone.workdate});
                ...
                if( M.modFlagOn('ciniki.wineproduction', 0x0200) ) {
                    ...
                    this.workdone.sections['racked'].num_cols = 9;
                    this.workdone.sections['racked'].headerValues = ['INV#', 'Wine', 'Type', 'SG', 'Started', 'BD', 'RD', '', ''];
                    this.workdone.sections['racked'].sortTypes = ['number', 'text', 'text', 'text', 'date', 'date', 'date', 'none'];
                    this.workdone.dataMaps['racked'] = ['invoice_number', 'wine_name', 'wine_type_and_length', 'sg_reading', 'start_date', 'bottling_date', 'racking_date', 'notes'];
                }
...
//              rsp = M.api.getJSON('ciniki.wineproduction.list', {'tnid':M.curTenantID, 'racking_date':this.workdone.workdate});
                ...
                if( M.modFlagOn('ciniki.wineproduction', 0x0200) ) {
                    ...
                    this.workdone.sections['filtered'].num_cols = 8;
                    this.workdone.sections['filtered'].headerValues = ['INV#', 'Wine', 'Type', 'Racked', 'BD', 'FD', ''];
                    this.workdone.sections['filtered'].sortTypes = ['number', 'text', 'text', 'date', 'date', 'date', 'none'];
                    this.workdone.dataMaps['filtered'] = ['invoice_number', 'wine_name', 'wine_type_and_length', 'rack_date', 'bottling_date', 'filtering_date', 'notes'];
                }
...
//              rsp = M.api.getJSON('ciniki.wineproduction.list', {'tnid':M.curTenantID, 'work_date':this.workdone.workdate});
        ...
        var rsp = M.api.getJSONCb('ciniki.wineproduction.list', 
            {'tnid':M.curTenantID, 'work_date':this.workdone.workdate}, function(rsp) {
                ...
                var p = M.ciniki_wineproduction_oldmain.workdone;
    ...
    this.showOrders = function(cb, ordertype) {
        if( cb != null ) {
            ...
            this.list.cb = 'M.ciniki_wineproduction_oldmain.showMain();';
        }
        var rsp = null;
        var rsp2 = null;
        var args = null;
        ...
        if( ordertype == 'start' || ordertype == 'ordered' ) {
            ...
            if( M.modFlagOn('ciniki.wineproduction', 0x0200) ) {
                ...
                this.list.sections['completed'].sortTypes = ['number', 'text', 'text', 'date', 'date', 'none'];
                this.list.dataMaps['completed'] = ['invoice_number_and_flags', 'wine_and_customer', 'wine_type_and_length', 'order_date', 'bottling_date_and_flags', 'notes'];
            }
...
//              rsp2 = M.api.getJSON('ciniki.wineproduction.list', {'tnid':M.curTenantID, 'status':'20', 'started_date':'today', 'sorting':'invoice_number'});
            ...
            if( M.modFlagOn('ciniki.wineproduction', 0x0200) ) {
                ...
                this.list.sections['pending'].num_cols = 5;
                this.list.sections['pending'].headerValues = ['INV#', 'Wine', 'Type', 'SG', ''];
                this.list.sections['pending'].sortTypes = ['number', 'text', 'text', 'number', 'none'];
                this.list.dataMaps['pending'] = ['invoice_number_and_flags', 'wine_and_customer', 'wine_type_and_length', 'sg_reading', 'sgbuttons'];
            }
            ...
            if( M.modFlagOn('ciniki.wineproduction', 0x0200) ) {
                ...
                this.list.sections['completed'].num_cols = 5;
                this.list.sections['completed'].headerValues = ['INV#', 'Wine', 'Type', 'SG', ''];
                this.list.dataMaps['completed'] = ['invoice_number_and_flags', 'wine_and_customer', 'wine_type_and_length', 'sg_reading', 'sgbuttons'];
            }
...
//              rsp2 = null;
            ...
            if( M.modFlagOn('ciniki.wineproduction', 0x0200) ) {
                ...
                this.list.sections['pending'].headerValues = ['INV#', 'Wine', 'Type', 'SG', 'Started', 'BD', 'RD', '', ''];
                this.list.sections['pending'].sortTypes = ['number', 'text', 'text', 'text', 'date', 'date', 'date', 'none', 'none'];
                this.list.dataMaps['pending'] = ['invoice_number_and_flags', 'wine_and_customer', 'wine_type_and_length', 'sg_reading', 'start_date', 'bottling_date_and_flags', 'racking_date', 'buttons', 'notes'];
                this.list.sections['completed'].num_cols = 8;
                this.list.sections['completed'].headerValues = ['INV#', 'Wine', 'Type', 'SG', 'Started', 'BD', 'FD', '', ''];
                this.list.sections['completed'].sortTypes = ['number', 'text', 'text', 'number', 'date', 'date', 'date', 'none'];
                this.list.dataMaps['completed'] = ['invoice_number_and_flags', 'wine_and_customer', 'wine_type_and_length', 'sg_reading', 'start_date', 'bottling_date_and_flags', 'filtering_date', 'notes'];
            }
        }
...
//              rsp2 = null;
            ...
            if( M.modFlagOn('ciniki.wineproduction', 0x0200) ) {
                ...
                this.list.sections['pending'].num_cols = 8;
                this.list.sections['pending'].sortTypes = ['number', 'text', 'text', 'date', 'date', 'date', 'none', 'none'];
                this.list.dataMaps['pending'] = ['invoice_number_and_flags', 'wine_and_customer', 'wine_type_and_length', 'rack_date', 'bottling_date_and_flags', 'filtering_date', 'buttons', 'notes'];
                this.list.sections['completed'].headerValues = ['INV#', 'Wine', 'Type', 'Racked', 'BD', 'FD', ''];
                this.list.sections['completed'].num_cols = 7;
                this.list.sections['completed'].sortTypes = ['number', 'text', 'text', 'date', 'date', 'date', 'none'];
                this.list.dataMaps['completed'] = ['invoice_number_and_flags', 'wine_and_customer', 'wine_type_and_length', 'rack_date', 'bottling_date_and_flags', 'filtering_date', 'notes'];
            }
        } 
...
//          rsp2 = null;
        ...
        var rsp = M.api.getJSONCb('ciniki.wineproduction.list', args, function(rsp) {
            ...
            var p = M.ciniki_wineproduction_oldmain.list;
        ...
        var rsp = M.api.getJSONCb('ciniki.wineproduction.actionOrder', 
            {'tnid':M.curTenantID, 'wineproduction_id':orderID, 'action':action, 
                'kit_length':kit_length, 'batch_code':batch_code}, function(rsp) {
                    ...
                    M.ciniki_wineproduction_oldmain.showOrders(null, M.ciniki_wineproduction_oldmain.list.ordertype);     
                });
    };
        ...
        var rsp = M.api.getJSONCb('ciniki.wineproduction.actionOrder', 
            {'tnid':M.curTenantID, 'wineproduction_id':orderID, 
                'action':action, 'kit_length':kit_length}, function(rsp) {
                    ...
                    M.ciniki_wineproduction_oldmain.showSchedule();        
            });
    };
    ...
    this.quickSGbutton = function(orderID, sg) {
        var rsp = M.api.getJSONCb('ciniki.wineproduction.actionOrder', 
            {'tnid':M.curTenantID, 'wineproduction_id':orderID, 'action':'SGRead', 'sg_reading':sg}, function(rsp) {
                ...
                M.ciniki_wineproduction_oldmain.showOrders(null, 'todayssgreadings');
            });
    };

    this.updateSG = function(section, r, c, d) {
        M.ciniki_wineproduction_oldmain.quickSGbutton(M.ciniki_wineproduction_oldmain.list.data[section][r].order.id, d);
    }
    ...
    this.showOrder = function(oid, cb) {
        ...
        var rsp = M.api.getJSONCb('ciniki.wineproduction.getOrder', 
            {'tnid':M.curTenantID, 'wineproduction_id':this.order.order_id}, function(rsp) {
                ...
                var p = M.ciniki_wineproduction_oldmain.order;
                p.data = rsp.order;
                p.customer_id = rsp.order.customer_id;
                if( rsp.order.customer_id > 0 ) {
                    p.sections.customer.addTxt = 'View Customer';
                    p.sections.customer.addFn = 'M.startApp(\'ciniki.customers.main\',null,\'M.ciniki_wineproduction_oldmain.showAppointment();\',\'mc\',{\'next\':\'M.ciniki_wineproduction_oldmain.updateAppointmentCustomer\',\'customer_id\':M.ciniki_wineproduction_oldmain.appointment.customer_id});';
                    p.sections.customer.changeTxt = 'Change Customer';
//                  p.data.customer = M.ciniki_wineproduction_oldmain.setupCustomer(rsp.order.customer);
                    p.data.customer = rsp.order.customer;
                } else {
                    p.sections.customer.addTxt = 'Add Customer';
                    p.sections.customer.addFn = 'M.startApp(\'ciniki.customers.edit\',null,\'M.ciniki_wineproduction_oldmain.showAppointment();\',\'mc\',{\'next\':\'M.ciniki_wineproduction_oldmain.updateAppointmentCustomer\',\'customer_id\':M.ciniki_wineproduction_oldmain.appointment.customer_id});';
    ...
    this.updateOrderCustomer = function(cid) {
        ...
        M.api.getJSONCb('ciniki.customers.customerDetails', {'tnid':M.curTenantID, 
            'customer_id':this.order.customer_id, 'phones':'yes', 'emails':'yes'}, function(rsp) {
                ...
                var p = M.ciniki_wineproduction_oldmain.order;
//              p.data.customer = M.ciniki_wineproduction_oldmain.setupCustomer(rsp.customer);
    ...
    this.showSchedule = function(cb, status, scheduleDate) {
        ...
        var rsp = M.api.getJSONCb('ciniki.wineproduction.statsSchedule', 
            {'tnid':M.curTenantID, 'start_date':this.schedule.date, 'days':14}, function(rsp) {
                ...
                var p = M.ciniki_wineproduction_oldmain.schedule;
        ...
        //          dt.setTime(dt.getTime()+86400000);
                ...
                M.ciniki_wineproduction_oldmain.showScheduleList(p.ordertype);
            });
    }
...
//              rsp = M.api.getJSON('ciniki.wineproduction.list', {'tnid':M.curTenantID, 'status_list':'20,25', 'before_racking_date':this.schedule.ordersDate + ' 12:00AM', 'sorting':'invoice_number'});
            ...
            if( M.modFlagOn('ciniki.wineproduction', 0x0200) ) {
                ...
                this.schedule.sections.orders.headerValues = ['INV#', 'Wine', 'Type', 'SG', 'Started', 'BD', 'RD', '', ''];
                this.schedule.sections.orders.sortTypes = ['number', 'text', 'text', 'number', 'date', 'date', 'date', 'none', 'none'];
                this.schedule.dataMaps.orders = ['invoice_number', 'wine_and_customer', 'wine_type_and_length', 'sg_reading', 'start_date', 'bottling_date_and_flags', 'racking_date', 'buttons', 'notes'];
            }
        } 
...
//              rsp = M.api.getJSON('ciniki.wineproduction.list', {'tnid':M.curTenantID, 'status':'30', 'before_filtering_date':this.schedule.ordersDate + ' 12:00AM', 'sorting':'invoice_number'});
            ...
            if( M.modFlagOn('ciniki.wineproduction', 0x0200) ) {
                ...
                this.schedule.sections.orders.sortTypes = ['number', 'text', 'text', 'date', 'date', 'date', 'none', 'none'];
                this.schedule.dataMaps.orders = ['invoice_number', 'wine_and_customer', 'wine_type_and_length', 'rack_date', 'bottling_date_and_flags', 'filtering_date', 'buttons', 'notes'];
            }
        } 
...
//      }
        ...
        if( c != '' ) {
            var rsp = M.api.postJSON('ciniki.wineproduction.update', 
                {'tnid':M.curTenantID, 'wineproduction_id':M.ciniki_wineproduction_oldmain.order.order_id}, c);
            if( rsp.stat != 'ok' ) {
                M.api.err(rsp);
                return false;
            } 
        }
        M.ciniki_wineproduction_oldmain.order.close();
    ...
    this.showAppointment = function(cb, aid) {
        ...
        var rsp = M.api.getJSONCb('ciniki.wineproduction.appointment', {'tnid':M.curTenantID, 
            'appointment_id':this.appointment.appointment_id}, function(rsp) {
                ...
                var p = M.ciniki_wineproduction_oldmain.appointment;
                ...
                for(i in rsp.appointments[0].appointment.orders) {
                    p.sections.wines.fields[i] = {'6':{'id':'order_' + rsp.appointments[0].appointment.orders[i].order.order_id + '_bottling_status', 'label':'Status', 'type':'flags', 'join':'yes', 'toggle':'yes', 'flags':M.ciniki_wineproduction_oldmain.bottlingStatus}};
                    for(var j in M.ciniki_wineproduction_oldmain.bottlingStatus) {
                        if( M.ciniki_wineproduction_oldmain.bottlingStatus[j].name == rsp.appointments[0].appointment.orders[i].order.bottling_status ) {
                ...
                if( p.customer_id > 0 ) {
                    ...
                    p.sections.customer.addFn = 'M.startApp(\'ciniki.customers.main\',null,\'M.ciniki_wineproduction_oldmain.showAppointment();\',\'mc\',{\'next\':\'M.ciniki_wineproduction_oldmain.updateAppointmentCustomer\',\'customer_id\':M.ciniki_wineproduction_oldmain.appointment.customer_id});';
                    p.sections.customer.changeTxt = 'Change Customer';
//                  p.data.customer = M.ciniki_wineproduction_oldmain.setupCustomer(rsp.appointments[0].appointment.customer);
        ...
        if( cid != null && this.appointment.customer_id != cid ) {
            ...
            var rsp = M.api.getJSONCb('ciniki.wineproduction.updateAppointment', 
                ...
                'customer_id':this.appointment.customer_id}, function(rsp) {
                    ...
                    M.ciniki_wineproduction_oldmain.updateAppointmentCustomerFinish();
                });
        } else {
        ...
        if( c != '' || bottled == 'yes' ) {
            ...
            var rsp = M.api.postJSONCb('ciniki.wineproduction.updateAppointment', 
                {'tnid':M.curTenantID, 'wineproduction_ids':wids, 'bottled':bottled}, c, function(rsp) {
                    ...
                    M.ciniki_wineproduction_oldmain.appointment.close();
                });
        } else {
            M.ciniki_wineproduction_oldmain.appointment.close();
        }
