...
function ciniki_wineproduction_searchProductNames($ciniki) {
	...
	if( $args['start_needle'] == '' && isset($args['customer_id']) && $args['customer_id'] > 0 ) {
		$strsql = "SELECT products.id, products.name AS wine_name, wine_type, kit_length "
			. "FROM wineproductions, products "
			. "WHERE wineproductions.business_id = '" . ciniki_core_dbQuote($ciniki, $args['business_id']) . "' "
			. "AND wineproductions.customer_id = '" . ciniki_core_dbQuote($ciniki, $args['customer_id']) . "' "
	...
	//
		...
		$strsql = "SELECT products.id, products.name AS wine_name, IFNULL(d1.detail_value, '') AS wine_type, IFNULL(d2.detail_value, '') AS kit_length "
			. "FROM products "
			...
			. "LEFT JOIN wineproductions ON (products.id = wineproductions.product_id "
				. "AND wineproductions.business_id = '" . ciniki_core_dbQuote($ciniki, $args['business_id']) . "') "
			. "WHERE products.business_id = '" . ciniki_core_dbQuote($ciniki, $args['business_id']) . "' "
			. "AND (products.name LIKE '" . ciniki_core_dbQuote($ciniki, $args['start_needle']) . "%' "	
	...
	$strsql .= "GROUP BY products.id "
		. "ORDER BY COUNT(wineproductions.id) DESC "
		. "";
