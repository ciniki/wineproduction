...
// <rsp stat='ok' id='34' />
//
function ciniki_wineproduction_productFileAdd(&$ciniki) {
    //  
    ...
    $rc = ciniki_wineproduction_checkAccess($ciniki, $args['tnid'], 'ciniki.wineproduction.productFileAdd'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    if( $rc['num_rows'] > 0 ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.wineproduction.166', 'msg'=>'You already have a file with this name, please choose another name'));
    }
    ...
    if( isset($_FILES['uploadfile']['error']) && $_FILES['uploadfile']['error'] == UPLOAD_ERR_INI_SIZE ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.wineproduction.144', 'msg'=>'Upload failed, file too large.'));
    }
    ...
    if( !isset($_FILES) || !isset($_FILES['uploadfile']) || $_FILES['uploadfile']['tmp_name'] == '' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.wineproduction.145', 'msg'=>'No file specified.'));
    }
    ...
    if( $args['extension'] != 'pdf' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.wineproduction.146', 'msg'=>'The file must be a PDF file.'));
    }
   
    //
    // Move the file to ciniki-storage
    //
    $storage_filename = $tenant_storage_dir . '/ciniki.wineproduction/files/' . $args['uuid'][0] . '/' . $args['uuid'];
    if( !is_dir(dirname($storage_filename)) ) {
        if( !mkdir(dirname($storage_filename), 0700, true) ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.wineproduction.147', 'msg'=>'Unable to add file'));
        }
    }

    if( !rename($_FILES['uploadfile']['tmp_name'], $storage_filename) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.wineproduction.165', 'msg'=>'Unable to add file'));
    }
    ...
    return ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.wineproduction.productfile', $args, 0x07);
}
?>
