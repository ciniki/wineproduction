...
// -------
//
function ciniki_wineproduction_supplierDelete(&$ciniki) {
    //
    ...
    $rc = ciniki_wineproduction_checkAccess($ciniki, $args['tnid'], 'ciniki.wineproduction.supplierDelete');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( !isset($rc['supplier']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.wineproduction.128', 'msg'=>'Supplier does not exist.'));
    }
    ...
    $strsql = "SELECT COUNT(id) AS num_items "
        ...
        . "WHERE supplier_id = '" . ciniki_core_dbQuote($ciniki, $args['supplier_id']) . "' "
    ...
    if( $rc['num'] > 0 ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.wineproduction.133', 'msg'=>'There are products for this supplier.'));
    }
    ...
    $rc = ciniki_core_objectCheckUsed($ciniki, $args['tnid'], 'ciniki.wineproduction.supplier', $args['supplier_id']);
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.wineproduction.129', 'msg'=>'Unable to check if the supplier is still being used.', 'err'=>$rc['err']));
    }
    if( $rc['used'] != 'no' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.wineproduction.130', 'msg'=>'The supplier is still in use. ' . $rc['msg']));
    }
    ...
    // Remove the supplier
    //
    $rc = ciniki_core_objectDelete($ciniki, $args['tnid'], 'ciniki.wineproduction.supplier',
