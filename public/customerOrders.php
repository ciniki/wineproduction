...
function ciniki_wineproduction_customerOrders($ciniki) {
    ...
    if( $rc['stat'] != 'ok' ) { 
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.wineproduction.14', 'msg'=>'Unable to understand request', 'err'=>$rc['err']));
    }   
    $args = $rc['args'];
    ...
    $strsql = "SELECT ciniki_wineproductions.id, "
        ...
        . "DATE_FORMAT(IF(rack_date > 0, DATE_ADD(rack_date, INTERVAL (ciniki_wineproduction_products.kit_length) DAY), "
            . "DATE_ADD(ciniki_wineproductions.start_date, INTERVAL ciniki_wineproduction_products.kit_length WEEK)), '" . ciniki_core_dbQuote($ciniki, $date_format) . "') AS approx_filtering_date "
    ...
    if( !isset($rc['orders']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.wineproduction.15', 'msg'=>'Unable to find any orders'));
    }
